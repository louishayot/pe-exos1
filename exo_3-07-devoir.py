...
# %%
#3
df = pd.DataFrame(Tableau, columns= ['names', 'speed', 'lifespan'])
df
...
# %%
#9
serie = pd.Series(df["speed"])
serie2 = pd.Series(df["lifespan"])
serie = serie.astype(float)
serie2 = serie2.astype(float)
df["speed"] = serie
df["lifespan"] = serie2
df
#df["lifespan"].dtype donne float64
...
# %%
df4.merge(df3)
#je ne suis pas sur d'avoir compris la comparaison contac et merge 
#parce que je ne parviens pas à aligner en "hauteur" avec merge
pd.merge(df3, df4)
...
# %%
df3i.reset_index(inplace=True)
df3i

# %%
df = pd.concat([df4i, df3i])
df.reset_index(drop=True, inplace=True)
df
